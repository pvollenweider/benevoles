...
const schema = z.object({
  ...
  // Optional override so a test can be previewed on any mailbox/client, not just the calling
  // super admin's own address — see #235.
...
export async function POST(req: Request) {
  ...
  const { subject, content, email } = parsed.data
  ...
  // The unsubscribe link always signs the calling super admin's own id, even when the test is
  // sent to a custom address — that address isn't necessarily a real AdminUser, and this is only
  // ever a preview (see the comment above), never an actual subscription to unsubscribe from.
  const unsubscribeUrl = `${BASE_URL}/api/public/product-updates/unsubscribe?admin=${self.id}&token=${unsubscribeToken(self.id)}`
  const result = await sendNotification({
    kind: "product_update",
    recipient: email ? { email, name: "Test" } : { email: self.email, name: self.name },
